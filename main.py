...
filename = 'nlp_model.pk1'
clf = pickle.load(open(filename, 'rb'))
vectorizer = pickle.load(open('trans.pk1', 'rb'))
...
def rcmd(m):
    ...
    try:
        ...
        print(f"Error in rcmd: {e}")
        data, similarity = create_similarity()
    if m not in data['movie_title'].unique():
        return 'Movie Title is not correct, Enter a valid name. THANKS FOR YOUR TIME !!'
    else:
...
def recommend():
    try:
        ...
        soup = bs.BeautifulSoup(sauce, 'html.parser')  # Changed 'lxml' to 'html.parser'
